...
%% Discription : 
  ...
  function [X]=IFSBLA(Y,A,Wgen,lambda_opt)
...
while ~converged
 ...
 for k= 1:K
     ...
     mu(:,k)=alpha*(L*mu(:,k)+Phi'*W1-B_k*B_k'*PhiTwo*mu(:,k))./(L*alpha+1./delta_inv);
     
     % factorization
     factorized_Sigma = 1./(alpha*(B_k*B_k')*diagPhiTwo+1./delta_inv);
     Exx(:,k)= mu(:,k).*conj(mu(:,k))+ factorized_Sigma;
 end
...
%   end
  ...
  for j=1:K
      %PGP=diag(Phi*diag(delta_inv)*Phi');
      bkbk=B(j,:)*B(j,:)';
      traceAGA1(:,j)=sum( diagPhiTwo./(alpha*bkbk*diagPhiTwo+1./delta_inv)   );
      traceAGA(:,j)= traceAGA1(:,j)  * bkbk ;
  end
...
mu_est(keep_list,:) = mu;
X=mu_est*B;
% subplot(1,2,1);plot(r1);subplot(1,2,2);plot(r2);
  end
